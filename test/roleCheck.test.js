const { servers, getAuthCookie, basicAuth, getJwtCookies } = require('./fixtures.cjs')
...
describe.each(servers)('rolecheck', (server) => {
  ...
  describe(`${server.name} hasAnyRole`, () => {
    it(`${server.name} allows users with any of the roles to access the end point`, async () => {
    ...
    it(`${server.name} denies users without the roles from using the end point`, async () => {
  ...
  describe(`${server.name} hasAllRoles`, () => {
    it(`${server.name} allows users with all roles to access the end point`, async () => {
      const jwtCookie1 = getAuthCookie(await getJwtCookies(baseUrl, basicAuth('donut', 'password321')))
    ...
    it(`${server.name} denies users with some matching roles to access the end point`, async () => {
      const jwtCookie = getAuthCookie(await getJwtCookies(baseUrl, basicAuth('taco', 'password123')))
      const res = await fetch(url('/secure/allRoles'), { headers: { Cookie: jwtCookie } })
      expect(res.status).toBe(403)
    })

    it(`${server.name} denies users with no matching roles to access the end point`, async () => {
      const jwtCookie = getAuthCookie(await getJwtCookies(baseUrl, basicAuth('rando', 'password313')))
      const res = await fetch(url('/secure/allRoles'), { headers: { Cookie: jwtCookie } })
      expect(res.status).toBe(403)
    })
